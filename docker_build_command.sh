...
docker run --rm -v ${PWD}:/root --security-opt seccomp:unconfined agodio/itba-so:1.0 make enable_buddy=1 -C /root
if [[ !  $? -eq 0 ]]
    then 
...
if [[ "$1" = "gdb" ]]
    then
    qemu-system-x86_64 -s -S -hda ./Image/x64BareBonesImage.qcow2 -m 512 1>/dev/null 2>&1 &
else
