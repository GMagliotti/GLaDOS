...
struct MemoryManager *buddy_new(void *const restrict memoryForMemoryManager, unsigned num_of_fragments, void * managedMemory)
{
    ...
    self = (struct MemoryManager *) memoryForMemoryManager;  
    self->start = managedMemory;                                   
 ...
 * than *size* */
unsigned choose_better_child(struct MemoryManager *self, unsigned index, size_t size)
{
    struct compound {
        size_t size;
        unsigned index;
...
//   buddy = buddy_new((void *)0x50000, (next_power_of_2(memoriaReservada) / 2) / fragmentSize, 0x1000000);
//   buddy_dump(buddy);
...
MemoryManagerADT createMemoryManager(void *const restrict memoryForMemoryManager, void *const restrict managedMemory){
    ...
    return buddy_new(memoryForMemoryManager, size / MMAN_PAGESIZE, managedMemory);
}

void *allocMemory(MemoryManagerADT const restrict memoryManager, const size_t memoryToAllocate){
    int offset = buddy_alloc(memoryManager, 
        (memoryToAllocate%MMAN_PAGESIZE == 0) ? memoryToAllocate/MMAN_PAGESIZE : memoryToAllocate/MMAN_PAGESIZE + 1);
    if (offset == -1) return NULL;
    return (void *)((char *)memoryManager->start + offset*MMAN_PAGESIZE);
}


void mman_free(struct MemoryManager * const restrict memoryManager, void * ptr) {
    buddy_free(memoryManager, ((char *)ptr-(char *)memoryManager->start)/MMAN_PAGESIZE);
}
