...
uint64_t create_sem(uint64_t initial_value, char *name) {
  ...
  if (sem_index == -1 || str_length(name) >= MAX_NAME)
    return -1;
  sem_t *sem = &(sem_spaces[sem_index].sem);
  if (initial_value == 0)
    sem->lock = 1;
  str_cpy(sem->name, name);
...
/**
 * @brief Decreases the value of the semaphore by one, or blocks the process if
 * the value is zero.
 * @param sem_idx The index of the semaphore
 * @return 1 if sem_idx is not valid, 0 otherwise
 */
...
int find_sem_index(char *name) {
  for (int i = 0; i < MAX_SEM; i++) {
    if (sem_spaces[i].available == FALSE &&
        (string_equals(sem_spaces[i].sem.name, name) == 1)) {
      return i;
    }
  }
  return -1;
