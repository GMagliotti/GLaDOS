...
void call_to_sys_read(uint8_t fd, uint8_t *to_ret, uint8_t cant_chars);
...
void call_to_clear_buffer();
void call_to_beep();
void call_to_fillrectangle(uint16_t x, uint16_t y, uint32_t color, uint16_t w,
                           uint16_t h);
void call_to_hlt();
int call_to_getticks();
void call_to_accessRTC(timeStructT *time_struct);
uint8_t call_to_memoryAt(int dir);
void call_to_set_size(int new_size);
void call_to_print_registers();

void *call_to_malloc(size_t requested_memory);
