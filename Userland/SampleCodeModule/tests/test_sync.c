...
uint64_t test_sync(uint64_t argc, char *argv[]) { //{n, use_sem, 0}
  ...
  char *argvDec[] = {"Decock", "-1", "1", NULL};
  char *argvInc[] = {"Incock", "1", "1", NULL};

  global = 0;

  uint64_t i;
  for (i = 0; i < pair_proc_num; i++) {
    pids[i] = call_to_create_process(3, argvDec, my_process_inc, NULL);
    pids[i + pair_proc_num] = call_to_create_process(3, argvInc, my_process_inc, NULL);
  }
